...
const getRideForTracking = async (req, res) => {
  try {
    const [ride] = await pool.query(
      'SELECT r.*, d.vehicle_number, u.name as driver_name, u.phone as driver_phone FROM rides r LEFT JOIN drivers d ON r.driver_id = d.id LEFT JOIN users u ON d.user_id = u.id WHERE r.id = ?',
      [req.params.rideId]
    );
